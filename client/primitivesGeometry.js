function setLShapePrimitiveVerticies(_x ,_y, unitStep) {
  var gl = glManager.gl
  var step = unitStep
  var x = _x
  var y = _y
  ...
  positions = positions.concat(unitBlock(x, y + (step + step), step))
  positions = positions.concat(unitBlock(x + step, y, step))

  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(positions), gl.STATIC_DRAW)
}
function setTShapePrimitiveVerticies(_x ,_y, unitStep) {
  var gl = glManager.gl
  var step = unitStep
  var x = _x
  var y = _y
